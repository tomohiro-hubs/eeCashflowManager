...
- 予定一覧のExcel出力（表示中の絞り込み結果を `.xlsx` で出力）
- 予定一覧の色ラベルに応じた行単位の塗りつぶし付きExcel出力
- Excel出力失敗時の停止箇所・詳細表示
...
### 予定一覧のExcel出力
- 予定一覧の `Excel出力` ボタンから、現在の絞り込み結果をそのまま `.xlsx` 形式でダウンロードできます
- 出力列は一覧の主要項目（ID、予定日、区分、CF区分、件名、内容、金額、メモ、入出金日、顧客名、担当社員名、完了状態、ラベル、管理番号）です
- 色ラベルが付いた行は、Excel上でも行全体を対応色で塗りつぶします
...
補足:
- `橙`、`紫`、`桃`、`灰` は現時点では塗りつぶし未対応です
- 空欄セルも行全体と同じ色で塗りつぶされます
- 出力失敗時は、画面上部に `停止箇所` と `詳細` を表示して原因切り分けできるようにしています
...
## 6. リリース一括スクリプト（ship.sh）
`scripts/ship.sh` は「デプロイ → コミット → push → PR作成 → マージ」を1コマンドでまとめて実行するスクリプトです。各ステップの前に確認プロンプトが出るので、途中でスキップもできます。

基本の使い方:
```bash
./scripts/ship.sh "fix: ボタンの並び順を調整"
```
第1引数がコミットメッセージ兼PRタイトルになります。

オプション:
- `--no-deploy`: 本番デプロイ（`npm run deploy`）をスキップ
- `--no-merge`: PR作成まで（マージしない）
- `--no-pr`: pushまで（PRを作らない）
- `--yes` / `-y`: 確認プロンプトを省略して自動実行
- `--help` / `-h`: ヘルプを表示

例:
```bash
./scripts/ship.sh --no-merge "wip: 表示調整"   # PR作成まで
./scripts/ship.sh -y "fix: 微修正"             # 確認なしで一気に実行
```

挙動と安全面:
- ステージ対象は `git add -u`（追跡済みファイルの変更のみ）。`.bak`・`Claude.pdf`・`.claude/` などの未追跡ファイルは対象外
- `main` ブランチ上では実行を拒否（作業ブランチ専用）
- コミットメッセージ末尾に `Co-Authored-By` を自動付与
- 同じブランチに既存のオープンPRがあれば、新規作成せず再利用
- ベースブランチは `main` を想定（スクリプト冒頭の `BASE_BRANCH` で変更可）

前提:
- GitHub CLI（`gh`）が認証済みであること（`gh auth status` で確認）
- デプロイには `wrangler` の認証・設定が有効であること

⚠️ 注意: PRのマージと本番デプロイは別物です。`ship.sh` は両方を順に実行しますが、`--no-deploy` や `--no-merge` で用途に応じて分けられます。
